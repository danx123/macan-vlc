...
# Menggantikan PyQt6.QtMultimedia dengan python-vlc
try:
    import vlc
except ImportError:
...
# --- KELAS-KELAS PEMBANTU (TIDAK ADA PERUBAHAN SIGNIFIKAN) ---
...
class MiniPlayerWindow(QWidget):
    ...
    def __init__(self, main_player_instance, parent=None):
        ...
        self.vlc_player = self.main_player.vlc_player # Referensi ke player VLC
        self.is_muted = self.vlc_player.audio_get_mute()
        self.last_volume = self.vlc_player.audio_get_volume() if not self.is_muted else 50
    ...
    def _setup_ui(self):
        ...
        # Gunakan QWidget biasa sebagai frame video
        ...
        self.volume_slider.setRange(0, 100) # Volume VLC standar 0-100
        self.volume_slider.setValue(self.vlc_player.audio_get_volume())
    ...
    def showEvent(self, event):
        # Arahkan output video VLC ke frame di mini player
        self.main_player._set_video_output(self.video_frame)
    ...
    def sync_volume_slider(self, volume, is_muted):
...
class ModernVideoPlayer(QWidget):
    ...
    volume_level_changed = pyqtSignal(int, bool)
    ...
    def _setup_themes(self):
        # (Kode tema tidak diubah, sama seperti file asli)
        self.themes = {
            "Dark": """
    ...
    def _setup_ui(self):
        ...
        # Gunakan QWidget biasa sebagai frame, bukan QVideoWidget
        self.video_frame = QWidget()
        self.video_frame.setStyleSheet("background-color: black;")
    ...
    def _connect_signals(self):
        ...
        # =============================================================================
        # FIX 2: Menambahkan koneksi untuk tombol Play/Pause, Stop, dan Slider di mini player
        # =============================================================================
    ...
    def _on_vlc_position_changed(self, event):
        if self.is_loading_media: # <-- TAMBAHKAN PENGECEKAN
            return
    ...
    def _load_video_file(self, file_path_or_url):
        ...
        # Buat media baru dari path/URL
        media = self.vlc_instance.media_new(file_path_or_url)
        self.vlc_player.set_media(media)
        
        # =============================================================================
        # FIX 1: Mengatur ulang volume player agar sesuai dengan UI sebelum play.
        # Ini mencegah slider reset ke posisi 0 saat memuat video baru.
        # =============================================================================
    ...
    def _sync_volume_slider(self):
        ...
        self._update_volume_icon()
        self.mini_player_widget.sync_volume_slider(vol, self.vlc_player.audio_get_mute())
    ...
    def _update_position(self, position):
        ...
        # Durasi mungkin belum siap saat posisi pertama kali update
    ...
    def _sync_main_volume_slider(self, volume, is_muted):
        if not self.volume_slider.isSliderDown():
            self.volume_slider.setValue(0 if is_muted else volume)
        self._update_volume_icon()
        self.mini_player_widget.sync_volume_slider(volume, is_muted)
    ...
    def _load_subtitle_file(self, video_path):
        ...
        if os.path.exists(srt_path):
            ...
            # VLC bisa auto-load, tapi custom parser memberi lebih banyak kontrol
        else:
